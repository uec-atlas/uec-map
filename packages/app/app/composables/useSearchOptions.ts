...
const searchOptions = [
    ...
    items: buildings
      .map((building) => ({
        ...
        value: {
          ...
          coordinate: centroid(
            polygonToLine(building.geometry as unknown as Polygon),
      ...
      .toSorted((a, b) => {
        const typeA =
          typeOrder[a.value.properties.type as keyof typeof typeOrder] ?? 999;
        const typeB =
          typeOrder[b.value.properties.type as keyof typeof typeOrder] ?? 999;
  ...
  },
  {
    ...
    items: gates.map((gate) => ({
      ...
      suffix:
        gate.properties.name === "中門"
          ? gate.properties.area === "east"
            ? "東地区"
            : "西地区"
          : undefined,
  ...
  },
  {
    ...
    items: floors.flatMap((room, index) => {
      ...
      if (
        !["lecture_room", "common_space", "office"].includes(
          room.properties.type,
        )
      )
        return [];
      const building = buildings.find(
        (b) => b.properties.id === room.properties.building_id,
      );
      if (!building) return [];
      return {
        id: `${index}`,
        label: name,
        suffix: room.properties.altname
          ? `${room.properties.altname}（${building.properties.name}）`
          : (building.properties.name as string),
        searchKey:
          `${building.properties.name} ${room.properties.name} ${room.properties.altname ?? ""} ${building.properties.name?.match(/.?[A-Z0-9]+/)?.[0] ?? ""}-${room.properties.name} ${building.properties.name?.match(/.?[A-Z0-9]+/)?.[0] ?? ""}${room.properties.name}`.trim(),
        icon:
          FLOOR_ICONS[room.properties.type as keyof typeof FLOOR_ICONS] ||
          FLOOR_ICONS.default,
        value: {
          type: "room",
          id: `${index}`,
          ...
          coordinate: centroid(
            polygonToLine(room.geometry as unknown as Polygon),
          ).geometry.coordinates,
...
const selectOptions = [searchOptions[0], searchOptions[1]].flatMap((group) =>
  ...
  return {
    ...
    fuseOptions: {
      keys: [
        ...
        "value.properties.altname",
        "value.properties.name:en",
        "value.properties.altname:en",
      ],
      threshold: 0.2,
    },
  };
